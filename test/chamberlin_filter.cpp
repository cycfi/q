...
TEST_CASE("chamberlin_modal_placement")
{
   // Placed by (f, tau), the impulse ring's bandpass envelope decays by
   // 1/e every tau. Kept well below the fs/6 stability ceiling.
   auto ring_peak_at = [](double f_hz, double tau_s, double t)
   {
      q::chamberlin_filter f{
         q::frequency(f_hz), sps, q::duration(tau_s)};
