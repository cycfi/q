...
void process(std::string name)
{
   ...
   q_test::write_golden_csv("results/golden/comp_ff_fb_" + name + ".csv", g_cols, g_rows);
   q_test::compare_golden_csv("comp_ff_fb_" + name, g_cols, g_rows);
}

TEST_CASE("compressor_ff_fb: audio files")
{
   process("1a-Low-E");
   process("Tapping D");
