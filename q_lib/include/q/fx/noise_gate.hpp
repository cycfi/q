...
namespace cycfi::q
{
  ...
  /*
   ...
   // size (in number of samples). Alternatively, an arbitrary non-zero
   // number may be used if you want to set the actual attack window in the
   ...
   namespace detail
   {
      ...
      struct noise_gate_base : slope
      {
         noise_gate_base() {}
         noise_gate_base(duration attack_width, float sps)
          : slope{attack_width, sps}
         {}
      };
   ...
   inline bool basic_noise_gate<attack_window>::operator()(float env)
   {
      ...
      {
         if (!_state && slope::operator()(env) > _onset_threshold)
            _state = 1;
         else if (_state && env < _release_threshold)
            _state = 0;
      }
