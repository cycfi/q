...
NOTE: The biquad is ideal for static filtering and EQ. For a filter whose cutoff is *modulated* per sample, such as a synth filter swept by an envelope or LFO, a direct-form biquad is not designed for fast time-varying coefficients and will produce artifacts; reach for the {resonant_filters} instead.
