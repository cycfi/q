...
`s`, `a`, `b`        :: Objects of type `Oscillator`.
...
| `Oscillator()`        | Default construct a `Oscillator`.
| `Oscillator(s)`       | Copy construct from `s`.
| `a = b`               | Assign `b` to `a`.
|===
...
| `s(pi)`      | Generate a periodic waveform given `{phase_iterator}`, `pi`.   | `decltype(s(pi))`
|===
...
`s`      :: Object of type `BasicOscillator`.
...
| `s(ph)`      | Generate a periodic waveform given `{phase}`, `ph`.            | `decltype(s(ph))`
|===
