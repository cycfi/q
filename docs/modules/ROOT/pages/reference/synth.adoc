...
`BandwidthLimitedOscillator` is a refinement of the `Oscillator` concept.
...
`Ramp` is a conceptual representation of a generator that generates signals from 0.0 to 1.0 or from 1.0 to 0.0. It is a refinement of the `Generator` concept. `Ramp`s are suitable for generating envelopes. A `Ramp` represents a segment of an envelope. Essentially, an envelope comprises two or more upward and downward `Ramp`s.

== Expressions

In addition to valid expressions for {Value}, `Ramp` allows these expressions.
