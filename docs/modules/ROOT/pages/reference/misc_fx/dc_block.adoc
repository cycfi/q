...
| `dc_block(rt)`        | Construct a `dc_block` with specified cutoff frequency, `f`, and samples
                          per second `sps`.
| `dc_block(b)`         | Copy construct a `dc_block` from `b`.
| `a = b`               | Assign `b` to `a`.
| `a = s`               | Set the current value to `s`.
|===
...
| `bk()`          | Return the current value.               | `float`
...
| `g.cutoff(f, sps)`    | Set the cutoff frequency given
