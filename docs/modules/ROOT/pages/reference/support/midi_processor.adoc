...
```c++
namespace cycfi::q::midi_1_0::concepts
{
   template <typename T>
   concept Processor =
...
`proc`  :: Object of type that conforms to the `Processor` concept.
`msg`   :: Object of type that conforms to the `Message` concept.
