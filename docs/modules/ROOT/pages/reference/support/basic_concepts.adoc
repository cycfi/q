...
`RandomAccessIteratable` is a concept that represents a random-access iteratable data type, which has `begin()` and `end()` member functions, and the return type of both functions is an iterator that conforms to the https://en.cppreference.com/w/cpp/iterator/random_access_iterator[`std::random_access_iterator`] concept.
