...
`audio_stream_base` is the abstract base class and the highest level interface for processing audio. It is responsible for setting up multichannel input and output buffers and presenting them to the user's processing function, which is called at regular intervals that are multiples of the sampling frequency.
