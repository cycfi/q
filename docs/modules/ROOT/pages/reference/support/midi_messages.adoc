...
MIDI messages are constructed from `raw_message`, which is essentially a `std::uint32_t` encapsulated in a `struct` that holds the 24-bits MIDI 1.0 message, encoded as little-endian:
...
The  `note_name` utility function can be used to convert a MIDI key number to a string representing the key.
...
Conversely, the `note_number` utility function can be used to convert a string representing the key to a MIDI key number.
