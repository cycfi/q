...
retuning a running generator glitch-free, but it also means a fresh
start requires an explicit `reset()`:

```c++
gen.config(width, sps);    // set the new width
gen.reset();               // start from the beginning
```

This matters most for the ramp generators below. The underlying raised
cosine is free-running: a completed ramp does not hold its end value,
and each completed ramp leaves the phase half a cycle on — so a
`config()`-only "restart" plays every other ramp *inverted*. The
generator has no notion of completion; run it for exactly one width's
worth of samples, then stop calling it.
