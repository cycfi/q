...
The Q DSP library also utilizes the exponential ramp generators for generating complex envelopes.
...
`exp_upward_ramp_gen` and `exp_downward_ramp_gen` are a models of `{generator}`. In addition to valid expressions for `{generator}`, `exp_upward_ramp_gen` and `exp_downward_ramp_gen` allow these expressions.
...
| `exp_upward_ramp_gen(w, sps)`        | Construct `exp_upward_ramp_gen` given window width, `w`,
                                         and samples per second, `sps`.
| `exp_upward_ramp_gen(w, sps, cv)`    | Construct `exp_upward_ramp_gen` given window width, `w`,
                                         and samples per second, `sps`, and exponential curvature, `cw`.
| `exp_downward_ramp_gen(w, sps)`      | Construct `exp_downward_ramp_gen` given window width, `w`,
                                         and samples per second, `sps`.
| `exp_downward_ramp_gen(w, sps, cv)`  | Construct `exp_downward_ramp_gen` given window width, `w`,
                                         and samples per second, `sps`, and exponential curvature, `cw`.
|===
...
| `g.config(w, sps, cw)`   | Set the window width, `w`, with samples per second, `sps`,
