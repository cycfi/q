...
image:expander.svg[alt="Expander", width=300, role=right]The `expander` is the inverse of the `compressor`. The `expander` adjusts the gain when the signal falls below the threshold, attenuating the signal. The `expander` is used to increase the dynamic range of an audio signal by reducing the level of low-level signals while leaving high-level signals unchanged, in contrast to a `compressor`, which reduces the level of high-level signals while leaving low-level signals unchanged.
