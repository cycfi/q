...
image:soft-knee-compressor.svg[alt="Soft Knee Compressor", width=300, role=right]The compressor dynamically modulates the gain when the signal envelope rises above a specified threshold. The compressor reduces the level of high-level signals (signals above the threshold) while leaving low-level signals unchanged. A type of compressor known as the soft knee compressor is implemented by a class named `soft_knee_compressor`. Envelope tracking is done externally using a separate envelope follower to make it possible to use different types of envelope tracking schemes, the output of which is the supplied argument to the function call operator.

Note that the `soft_knee_compressor` is an envelope processor that operates on the envelope of the audio signal, perform computations in the logarithmic domain, and returns a processed envelope.
...
`comp`, `a`, `b`     :: Instances of `soft_knee_compressor`
`t`, `w`, `env`      :: Instances of `decibel`
...
| `soft_knee_compressor(c)`      | Copy construct from `soft_knee_compressor c`.
| `a = b`                        | Assign `b`, to `a`.
|===
