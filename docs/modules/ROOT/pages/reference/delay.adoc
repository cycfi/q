...
`basic_delay` is a basic class for delays. The actual delay duration parameter is decoupled from, and managed outside the class to allow both single and multi-tapped delays.

`basic_delay` is a template class with a `Base` template parameter, which is expected to be the ring buffer storage type from which it inherits from. See {ring_buffer} and {fractional_ring_buffer}
