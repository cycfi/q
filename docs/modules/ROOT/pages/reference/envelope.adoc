...
The Q DSP library includes a variety of envelope extractors, ranging from the most basic peak envelope follower, through the fast follower family for low-ripple transient detection, to the true RMS envelope follower, which measures actual signal power independent of the waveform's crest factor. Each has distinct strengths, characteristics, and tradeoffs that can be evaluated based on the application's specific requirements.
