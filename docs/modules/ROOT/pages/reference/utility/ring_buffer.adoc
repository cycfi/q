...
`rb`, `a`, `b`    :: Instances of `ring_buffer<T, S>`.
`val`             :: Instance of `T`.
`i`               :: Instance of `std::size_t`.
`s`               :: Instance of `std::size_t`.
...
| `rb.push(val)`     | Push `val` into the ring buffer's
                       front. This operation will
                       overwrite the oldest element in
                       the buffer.                       | `void`
| `rb.clear()`       | Clear the ring buffer.            | `void`
...
| `rb.front()`       | Get the latest element.           | `T&` or `T const&` if ring
                                                            buffer is const.
| `rb.back()`        | Get the oldest element.           | `T&` or `T const&` if ring
                                                           buffer is const.
| `rb[i]`            | Get the element at index i. `rb[0]`
                       refers to the latest element.
                       `rb[rb.size()-1]` refers to the
                       oldest element.                   | `T&` or `T const&` if ring
                                                           buffer is const.
| `rb.store()`       | Get a reference to the storage.   | `S&` or `S const&` if ring
                                                            buffer is const.
|===
