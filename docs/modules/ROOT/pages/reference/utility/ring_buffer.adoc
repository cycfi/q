...
| `ring_buffer<T>`               | Instantiate a `ring_buffer` type given the
                                   element type `T`.
| `ring_buffer<T, S>`            | Instantiate a `ring_buffer` type given the
                                   element type `T` and storage type `S`.
|===
...
| `rb.push(val)`     | Push the latest element,
                       overwriting the oldest element.   | `void`
