...
`T`      :: underlying type of `basic_interval<T>`.
`T2`     :: underlying type of `basic_interval<T2>`.
`a`, `i` :: Object of type `basic_interval<T>`.
`b`      :: Object of type `basic_interval<T2>`.
`v`      :: Arithmetic value.
...
| `basic_interval<T>(v)`   |  Construct a `basic_interval<T>` from an arithmetic value `v`.
| `interval(v)`            |  Equivalent to `basic_interval<double>(v)`.
| `exact_interval(v)`      |  Equivalent to `basic_interval<std::int8_t>(v)`.
| `basic_interval<T>(b)`   |  Copy construct from `basic_interval<T2> b`.
| `a = b`                  |  Assign `basic_interval<T2> b`, to `basic_interval<T> a`.
|===
...
| `as_int(i)`     | Convert a `i` to an `int`.     | `int`
| `as_float(i)`   | Convert a `i` to a `float`.    | `float`
| `as_double(i)`  | Convert a `i` to a `double`.   | `double`
|===
...
| `round(i)`         | Round `i` to the nearest non-fractional
