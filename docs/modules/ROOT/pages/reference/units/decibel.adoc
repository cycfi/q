...
`dbv`    :: Floating point value in decibels.
`0.0`    :: A floating point literal number.
`0`      :: An integer literal number.

=== Constructor

The `decibel` constructor from a floating point value is deleted:

```c++
auto db = decibel{6.0}; // error
```
...
Use `dB` to create a `decibel` object from a runtime value that is already expressed in decibels:

```c++
auto value = read_decibel_value();
auto db = dB(value);
```
...
| `dB(dbv)`          | Convert a dB value `dbv` to `decibel`.     | `decibel`
...
Use `dB` when the input value is already expressed in decibels. Example:
