...
Take note that the `{Unit}` concept already has a constructor from an arithmetic value that handles direct construction such as:
...
| `lin2db(v)`        | Convert a linear value `v` to a
                       `decibel` object from.                     | `decibel`
| `approx_db(v)`     | Faster approximate conversion from
                       a linear value `v` to `decibel`.           | `decibel`
|===

The free function `lin2db` can be used to indirectly construct and return a `decibel` object from a linear value. The `lin2db` function performs linear to decibel conversion. Example:

```c+
auto db = lin2db(2.0);  // 6 dB
```

Another free function `approx_db` is a faster approximate conversion from a linear value to `decibel`. `approx_db` also performs linear to decibel conversion.

`lin_float` and `lin_double` convert `decibel` to a linear value.
