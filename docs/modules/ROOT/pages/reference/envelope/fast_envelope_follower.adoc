...
The output is suitable for signal analysis as is. If the signal is to be utilized for modulation, for example as input to a {compressor}, further filtering is recommended. {smoothed_fast_envelope_follower} is one such example.
