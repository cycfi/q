...
The Fast RMS Envelope Follower is a variation of the {fast_envelope_follower}. It is a simple composite class that contains a {smoothed_fast_envelope_follower} member.

image:rms.svg[alt="RMS", width=300, role=right]The signal path is as follows:

1. Square the signal.
2. Apply the smoothed fast envelope follower. The moving average filter in the smoothed fast envelope follower returns the arithmetic mean of the squared values.
3. Return the square root the result.

This is embodied in the `fast_rms_envelope_follower` class. Following the fast envelope follower API, the function call operator accepts a scalar sample, transforms it, and returns an envelope, also scalar.
...
The plot in Figure 1 below shows the fast RMS envelope follower’s response (envelope: green, signal: blue, absolute value of the signal: dark blue). It may look indistinguishable from the {smoothed_fast_envelope_follower}'s output, but this detector returns true RMS results.
...
`env`, `a`, `b`     :: Instances of `fast_rms_envelope_follower` or `fast_rms_envelope_follower_db`.
