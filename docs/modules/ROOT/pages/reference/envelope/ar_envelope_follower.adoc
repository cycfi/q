...
`env`, `a`, `b`     :: Instances of `ar_envelope_follower`
`atk`, `rel`        :: Instances of `duration`
...
| `ar_envelope_follower(atk, rel, sps)` | Construct a `ar_envelope_follower` given `atk` (attack duration), `rel` (release duration) and `sps` (samples per second).
