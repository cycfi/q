...
NOTE: The full example can be found here:
https://github.com/cycfi/Q/blob/master/example/square_osc.cpp[example/square_osc.cpp].
...
NOTE: For now, we will skim over details such as the `envelope`, `phase`, and `phase_iterator`, and  and this thing called `poly blep`. The important point, exemplified here, is that we want to keep our building blocks as simple and minimal as possible. We will cover that in greater detail later.
